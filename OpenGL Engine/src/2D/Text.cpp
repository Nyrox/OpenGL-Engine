...
Text::Text() {
	...
	gl::VertexAttribPointer(0, 2, gl::FLOAT, false, 2 * sizeof(GLfloat), 0);
	gl::EnableVertexAttribArray(0);
	gl::VertexAttribPointer(1, 2, gl::FLOAT, false, 2 * sizeof(GLfloat), (void*)(sizeof(GLfloat) * 2));
...
void Text::render() {
	...
	for (auto& it : buffer) {
		...
		float yFinal = yPosition - (character.size.y - character.bearing.y);
